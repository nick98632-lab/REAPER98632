# =============================================================================
# SEQUENCE MANUSCRIPT PIPELINE (CHECK 10 REPAIR)
# =============================================================================
# Core repairs in this version:
# 1. Uses empirical p-values from fdrtool for HC thresholding and HBFSS.
# 2. Weak DC2 calls are gated by the empirical HC threshold, not a Wald HC line.
# 3. Shared EVS cutoff uses the LAST stable crossing within the allowed range.
# 4. Shared cutoff is applied on the combined rank so the leading edge ends at the cutoff.
# 5. Volcano legends use colored markers that match the plotted classes.
# 6. Treatment, control, and shared cutoff panels are exported.
# 7. Script writes to a stable exports folder inside the repository.
# =============================================================================
...
crossing_min_percentile <- 0.75
crossing_max_percentile <- 0.99
crossing_stability_window_n <- 3L
crossing_rule <- "last_stable_crossing"
...
assert_required_columns <- function(df, required_cols, object_name = "data frame") {
  ...
  if (length(missing_cols) > 0) {
    stop(paste0("Missing required columns in ", object_name, ": ", paste(missing_cols, collapse = ", ")), call. = FALSE)
  }
}
...
compact_title <- function(x, width = 58) paste(strwrap(as.character(x), width = width), collapse = "\n")
compact_caption <- function(x, width = 120) paste(strwrap(as.character(x), width = width), collapse = "\n")

strip_dataset_key <- function(dataset_name) sub("^.*_(raw_dataset|leading_edge_dataset|remainder_dataset)$", "\\1", dataset_name)
comparison_label_from_dataset_name <- function(dataset_name) sub("_(raw_dataset|leading_edge_dataset|remainder_dataset)$", "", dataset_name)
pretty_dataset_type <- function(dataset_key) dataset_key_labels[[dataset_key]]
pretty_dataset_label <- function(dataset_name) {
  if (!grepl("_(raw_dataset|leading_edge_dataset|remainder_dataset)$", dataset_name)) return(dataset_name)
  paste(comparison_label_from_dataset_name(dataset_name), pretty_dataset_type(strip_dataset_key(dataset_name)), sep = " | ")
}
pretty_group_label <- function(group_label) switch(as.character(group_label), trt = "Treatment", untrt = "Control", treatment = "Treatment", control = "Control", as.character(group_label))

save_csv <- function(df, path) utils::write.csv(df, file = path, row.names = FALSE)

save_grob <- function(g, path, width = 8.8, height = 6.5, dpi = figure_dpi, bg = "white") {
  if (is.null(g)) return(invisible(FALSE))
  ggplot2::ggsave(filename = path, plot = g, width = width, height = height, dpi = dpi, units = "in", bg = bg, limitsize = FALSE)
  invisible(TRUE)
}
...
make_design_formula <- function() ~ condition
get_condition_coef <- function(dds) { rn <- resultsNames(dds); idx <- grep("^condition_", rn); if (!length(idx)) stop("Could not identify condition coefficient.", call. = FALSE); rn[idx[1]] }
clean_gene_set <- function(x) unique(tolower(trimws(x[!is.na(x) & x != ""])))
...
run_empirical_null_fdrtool <- function(stat_vec, dataset_name) {
  ...
  if (length(stat_vec) < 5) stop(sprintf("[%s] Too few finite Wald statistics for empirical-null fitting.", dataset_name), call. = FALSE)
  fit <- tryCatch(
...
safe_hc_thresh <- function(p_vec, dataset_name, upper_cap = max_usable_hc_p_threshold) {
  ...
  out <- suppressWarnings(tryCatch(fdrtool::hc.thresh(as.vector(sorted_p)), error = function(e) {
    message(sprintf("[%s] hc.thresh failed: %s", dataset_name, conditionMessage(e)))
    NA_real_
  }))
  out <- as.numeric(out[1])
  if (!is.finite(out) || is.na(out) || out <= 0 || out >= upper_cap) return(NA_real_)
  out
}

resolve_empirical_hc_threshold <- function(empirical_p, dataset_name) {
  hc <- safe_hc_thresh(empirical_p, dataset_name = paste(dataset_name, "empirical", sep = " | "))
  if (is.finite(hc) && !is.na(hc)) return(list(threshold = hc, source = "empirical_hc_primary"))
  p_valid <- clip_probabilities(empirical_p)
  p_valid <- p_valid[is.finite(p_valid) & !is.na(p_valid)]
  if (length(p_valid) >= 5) {
    fallback_hc <- as.numeric(stats::quantile(p_valid, probs = alpha_level, na.rm = TRUE, type = 7)[1])
    if (is.finite(fallback_hc) && !is.na(fallback_hc) && fallback_hc > 0 && fallback_hc < max_usable_hc_p_threshold) {
      return(list(threshold = fallback_hc, source = "empirical_quantile_fallback"))
    }
  }
  list(threshold = NA_real_, source = "empirical_missing")
}
...
prepare_comparison_data <- function(comparison_name, group1_prefix, group2_prefix, WTTS_Seq, meta_all) {
  keep_ids <- grepl(paste0("^", group1_prefix, "_"), meta_all$id) | grepl(paste0("^", group2_prefix, "_"), meta_all$id)
  ...
  if (length(missing_samples) > 0) stop(paste("Missing samples in WTTS file for", comparison_name, ":", paste(missing_samples, collapse = ", ")), call. = FALSE)
...
compute_pc1_loading_table <- function(value_df, sample_names, preprocessing_label, feature_metrics = NULL) {
  ...
  x_log <- log2(x + 1)
  row_vars <- apply(x_log, 1, var)
  keep_rows <- is.finite(row_vars) & row_vars > 0
  if (sum(keep_rows) < 5L) stop("Fewer than 5 features with nonzero variance remain after filtering for PCA.", call. = FALSE)
  ...
  loading_tbl <- data.frame(feature_id = names(loading_abs), pc1_loading = unname(pca_fit$rotation[, 1]), pc1_loading_abs = unname(loading_abs), stringsAsFactors = FALSE)
  if (!is.null(feature_metrics) && nrow(feature_metrics) > 0) {
    ...
    loading_tbl <- left_join(loading_tbl, fm, by = "feature_id")
  }
  ...
  list(pca_fit = pca_fit, loading_table = loading_tbl, cutoff = NA_real_, top_n_used = NA_integer_, cutoff_quantile = NA_real_, cutoff_method = "shared_cutoff_pending", selected_reason = "shared_cutoff_pending", preprocessing_label = preprocessing_label)
}
...
build_ranked_fourier_metric_table <- function(loading_tbl, mean_col = "baseMean", dispersion_col = "dispGeneEst") {
  ...
  df <- df[is.finite(df$mean_value) & !is.na(df$mean_value) & df$mean_value > 0 & is.finite(df$dispersion_value) & !is.na(df$dispersion_value) & df$dispersion_value > 0, , drop = FALSE]
...
build_percentile_windows_fourier <- function(n_total, step = fourier_percentile_step, window_fraction = fourier_window_fraction, min_rank = fourier_min_rank, max_rank_frac = fourier_max_rank_frac) {
  ...
  data.frame(percentile = pct_grid, center_rank = center_ranks, lo_rank = pmax(1L, center_ranks - half_window), hi_rank = pmin(n_total, center_ranks + half_window), stringsAsFactors = FALSE)
}
...
fit_local_fourier <- function(rank_vec, y_vec, n_harmonics = fourier_harmonics) {
  ...
  list(fit = fit, fitted_y = fitted_y, residual_y = residual_y, local_amplitude = local_amplitude, peak_rank = rank_vec[peak_idx], trough_rank = rank_vec[trough_idx], center_rank = center_rank, residual_sd = stats::sd(residual_y, na.rm = TRUE))
}
...
summarize_local_fourier_window <- function(metric_df, lo_rank, hi_rank) {
  ...
  if (nrow(sub) < 9L) return(data.frame(iod_amplitude = NA_real_, cv2_amplitude = NA_real_, iod_center = NA_real_, cv2_center = NA_real_, iod_residual_sd = NA_real_, cv2_residual_sd = NA_real_, stringsAsFactors = FALSE))
  iod_fit <- fit_local_fourier(sub$rank, sub$log_iod_nb)
  cv2_fit <- fit_local_fourier(sub$rank, sub$log_cv2_nb)
  if (is.null(iod_fit) || is.null(cv2_fit)) return(data.frame(iod_amplitude = NA_real_, cv2_amplitude = NA_real_, iod_center = NA_real_, cv2_center = NA_real_, iod_residual_sd = NA_real_, cv2_residual_sd = NA_real_, stringsAsFactors = FALSE))
  data.frame(iod_amplitude = iod_fit$local_amplitude, cv2_amplitude = cv2_fit$local_amplitude, iod_center = iod_fit$center_rank, cv2_center = cv2_fit$center_rank, iod_residual_sd = iod_fit$residual_sd, cv2_residual_sd = cv2_fit$residual_sd, stringsAsFactors = FALSE)
}
...
build_local_fourier_wave_map <- function(loading_tbl, mean_col = "baseMean", dispersion_col = "dispGeneEst") {
  ...
  wave_map <- bind_rows(rows)
  iod_amp_scaled <- if (all(is.na(wave_map$iod_amplitude))) rep(NA_real_, nrow(wave_map)) else scales::rescale(wave_map$iod_amplitude, to = c(0, 1), from = range(wave_map$iod_amplitude, na.rm = TRUE))
  cv2_amp_scaled <- if (all(is.na(wave_map$cv2_amplitude))) rep(NA_real_, nrow(wave_map)) else scales::rescale(wave_map$cv2_amplitude, to = c(0, 1), from = range(wave_map$cv2_amplitude, na.rm = TRUE))
  ...
  wave_map$local_fourier_score <- fourier_score_weight_iod_amp * iod_amp_scaled + fourier_score_weight_cv2_amp * cv2_amp_scaled + fourier_score_weight_center_agreement * wave_map$center_agreement
...
combine_treatment_control_fourier_maps <- function(trt_wave_obj, ctrl_wave_obj) {
  ...
  out <- inner_join(trt_map2, ctrl_map2, by = "percentile")
  ...
  out$combined_center_distance <- abs(((out$iod_center_trt + out$iod_center_ctrl) / 2) - ((out$cv2_center_trt + out$cv2_center_ctrl) / 2))
  out$combined_center_agreement <- 1 / (1 + out$combined_center_distance)
  iod_amp_scaled <- if (all(is.na(out$combined_iod_amplitude))) rep(NA_real_, nrow(out)) else scales::rescale(out$combined_iod_amplitude, to = c(0, 1), from = range(out$combined_iod_amplitude, na.rm = TRUE))
  cv2_amp_scaled <- if (all(is.na(out$combined_cv2_amplitude))) rep(NA_real_, nrow(out)) else scales::rescale(out$combined_cv2_amplitude, to = c(0, 1), from = range(out$combined_cv2_amplitude, na.rm = TRUE))
  out$combined_fourier_score <- fourier_score_weight_iod_amp * iod_amp_scaled + fourier_score_weight_cv2_amp * cv2_amp_scaled + fourier_score_weight_center_agreement * out$combined_center_agreement
  out
}

compute_regime_difference_curve <- function(combined_wave_df) {
  df <- as.data.frame(combined_wave_df, stringsAsFactors = FALSE)
  assert_required_columns(df, c("percentile", "combined_center_rank", "combined_iod_amplitude", "combined_cv2_amplitude"), object_name = "combined_wave_df")
  df$regime_difference <- df$combined_iod_amplitude - df$combined_cv2_amplitude
  df$regime_direction <- ifelse(is.na(df$regime_difference), NA_character_, ifelse(df$regime_difference > 0, "IOD_dominant", ifelse(df$regime_difference < 0, "CV2_dominant", "balanced")))
  df
}
...
find_crossing_intervals <- function(diff_df, min_percentile = crossing_min_percentile, max_percentile = crossing_max_percentile) {
  ...
  keep <- is.finite(df$percentile) & !is.na(df$percentile) & is.finite(df$regime_difference) & !is.na(df$regime_difference) & df$percentile >= min_percentile & df$percentile <= max_percentile
  df <- df[keep, , drop = FALSE]
  if (nrow(df) < 2L) return(data.frame())
  rows <- list(); kk <- 1L
  for (i in seq_len(nrow(df) - 1L)) {
    y1 <- df$regime_difference[i]; y2 <- df$regime_difference[i + 1L]
    ...
    crossing_percentile <- x2
    crossing_rank <- r2
    rows[[kk]] <- data.frame(crossing_id = paste0("crossing_", kk), idx_left = i, idx_right = i + 1L, percentile_left = x1, percentile_right = x2, rank_left = r1, rank_right = r2, regime_difference_left = y1, regime_difference_right = y2, crossing_percentile = crossing_percentile, crossing_rank = as.integer(crossing_rank), stringsAsFactors = FALSE)
    kk <- kk + 1L
  }
...
label_stable_crossings <- function(diff_df, crossing_tbl, stability_window_n = crossing_stability_window_n) {
  ...
  for (i in seq_len(nrow(out))) {
    ...
    out$stable_crossing[i] <- isTRUE(is.finite(left_pos_frac) && is.finite(right_neg_frac) && left_pos_frac >= 0.67 && right_neg_frac >= 0.67)
  }
  out
}
...
select_regime_shift_crossing <- function(combined_wave_df) {
  ...
  if (!nrow(crossing_tbl)) return(list(diff_df = diff_df, crossing_table = crossing_tbl, selected_crossing = NULL, selected_reason = "no_crossings_found"))
...
resolve_top_n_cutoff <- function(sorted_values_desc, top_n = evs_fixed_top_n) {
  ...
  list(top_n_actual = top_n_actual, cutoff_value = cutoff_value, cutoff_quantile = cutoff_quantile, n_total = n_total, method = "fixed_top_n", candidate_table = data.frame(candidate_id = "fixed_top_n", rank_index = top_n_actual, cutoff_value = cutoff_value, cutoff_quantile = cutoff_quantile, selected = TRUE, selected_reason = "fixed_top_n", stringsAsFactors = FALSE), selected_reason = "fixed_top_n")
}
...
resolve_combined_fourier_cutoff <- function(fit_trt_loading_tbl, fit_ctrl_loading_tbl, fixed_top_n = evs_fixed_top_n) {
  ...
  candidate_table <- data.frame(candidate_id = selected_crossing$crossing_id[1], rank_index = selected_rank, percentile = selected_crossing$crossing_percentile[1], cutoff_value = cutoff_value, cutoff_quantile = cutoff_quantile, selected = TRUE, selected_reason = crossing_info$selected_reason, stringsAsFactors = FALSE)
  list(top_n_actual = selected_rank, cutoff_value = cutoff_value, cutoff_quantile = cutoff_quantile, n_total = n_total, method = crossing_info$selected_reason, trt_wave_obj = trt_wave_obj, ctrl_wave_obj = ctrl_wave_obj, shared_combined_tbl = shared_combined_tbl, combined_wave_map = crossing_info$diff_df, crossing_table = crossing_info$crossing_table, selected_crossing = selected_crossing, candidate_table = candidate_table, selected_reason = crossing_info$selected_reason)
}

select_wave_backup_cutoff <- function(wave_obj, group_label = "group") {
  if (is.null(wave_obj) || is.null(wave_obj$wave_map) || !nrow(wave_obj$wave_map)) return(list(diff_df = data.frame(), crossing_table = data.frame(), selected_crossing = NULL, selected_reason = "wave_map_missing"))
  df <- as.data.frame(wave_obj$wave_map, stringsAsFactors = FALSE)
  diff_df <- data.frame(percentile = as.numeric(df$percentile), combined_center_rank = as.integer(round(df$center_rank)), regime_difference = as.numeric(df$iod_amplitude - df$cv2_amplitude), stringsAsFactors = FALSE)
  crossing_tbl <- find_crossing_intervals(diff_df)
  crossing_tbl <- label_stable_crossings(diff_df, crossing_tbl)
  if (nrow(crossing_tbl)) {
    ...
    if (nrow(stable_tbl)) {
      selected <- stable_tbl[order(stable_tbl$crossing_percentile, decreasing = TRUE), , drop = FALSE][1, , drop = FALSE]
      reason <- paste0(group_label, "_last_stable_crossing")
    } else {
      selected <- crossing_tbl[order(crossing_tbl$crossing_percentile, decreasing = TRUE), , drop = FALSE][1, , drop = FALSE]
      reason <- paste0(group_label, "_last_crossing_fallback")
    }
...
apply_loading_cutoff <- function(fit_obj, rank_index, selected_reason = "manual_override") {
  ...
  rank_index <- min(max(1L, as.integer(rank_index)[1]), n_total)
...
build_eigenvector_split <- function(count_matrix, coldata, comparison_name) {
  ...
  combined_cutoff_info <- resolve_combined_fourier_cutoff(fit_trt_loading_tbl = fit_trt$loading_table, fit_ctrl_loading_tbl = fit_untrt$loading_table, fixed_top_n = evs_fixed_top_n)
  ...
  if (is.null(shared_combined_tbl) || !nrow(shared_combined_tbl)) stop("Shared combined loading table is missing after shared cutoff resolution.", call. = FALSE)
  ...
  if (!length(leading_edge_ids)) stop("Leading-edge dataset is empty.", call. = FALSE)
  if (!length(remainder_ids)) stop("Remainder dataset is empty.", call. = FALSE)
  evs_cutoff_summary <- bind_rows(
    data.frame(preprocessing = "normalized", group = "regime_shift_crossing", cutoff_mode = combined_cutoff_info$method, fixed_top_n_requested = evs_fixed_top_n, empiric_rank_selected = final_shared_rank, cutoff_quantile = fit_trt$cutoff_quantile, selected_reason = combined_cutoff_info$selected_reason, stringsAsFactors = FALSE),
    data.frame(preprocessing = "raw", group = "regime_shift_crossing", cutoff_mode = "crossing_rank_projected_to_raw_selected", fixed_top_n_requested = evs_fixed_top_n, empiric_rank_selected = final_shared_rank, cutoff_quantile = fit_trt_raw$cutoff_quantile, selected_reason = paste0(combined_cutoff_info$selected_reason, "_projected_to_raw"), stringsAsFactors = FALSE)
  )
  list(fit_trt = fit_trt, fit_untrt = fit_untrt, fit_trt_raw = fit_trt_raw, fit_untrt_raw = fit_untrt_raw, combined_cutoff_info = combined_cutoff_info, shared_combined_tbl = shared_combined_tbl, evs_cutoff_summary = evs_cutoff_summary, normalized_counts = norm_counts_init, raw_dataset = count_matrix, leading_edge_dataset = count_matrix[leading_edge_ids, , drop = FALSE], remainder_dataset = count_matrix[remainder_ids, , drop = FALSE])
}
...
make_pca_summary_table <- function(pca_fit, dataset_name, preprocessing_label, group_label = NA_character_) {
  ...
  data.frame(dataset_name = dataset_name, preprocessing_label = preprocessing_label, group_label = group_label, principal_component = paste0("PC", seq_along(sdev)), standard_deviation = as.numeric(sdev), variance = as.numeric(variance), proportion_variance = as.numeric(prop_var), cumulative_proportion = as.numeric(cum_var), stringsAsFactors = FALSE)
}
...
plot_fourier_wave_map_single <- function(wave_obj, comparison_name, group_label) {
  ...
  p <- ggplot(df, aes(percentile)) +
    ...
    geom_line(aes(y = cv2_amplitude, color = "CV2"), linewidth = 0.9) +
    scale_color_manual(values = c("IOD" = plot_palette$treatment, "CV2" = plot_palette$control)) +
    labs(title = paste0(pretty_group_label(group_label), " | local regime lines"), subtitle = compact_caption("This panel includes the backup within-group cutoff.", width = 88), x = "Percentile center", y = "Local amplitude", color = NULL) +
    manuscript_theme()
  if (is.finite(cutoff_pct)) {
    ymax <- max(c(df$iod_amplitude, df$cv2_amplitude), na.rm = TRUE)
    p <- p + geom_vline(xintercept = cutoff_pct, linetype = "dotted", linewidth = 0.9, colour = line_col) + annotate("label", x = cutoff_pct, y = ymax, label = paste0(pretty_group_label(group_label), " backup\np = ", signif(cutoff_pct, 4), "\nrank = ", cutoff_rank), fill = "white", colour = line_col, size = 2.8, label.size = 0.15, vjust = -0.5)
  }
  p
}
...
plot_combined_fourier_wave_map <- function(combined_cutoff_info, comparison_name) {
  ...
  p <- ggplot(df, aes(percentile)) +
    ...
    geom_line(aes(y = combined_cv2_amplitude, color = "Composite CV2"), linewidth = 0.95) +
    scale_color_manual(values = c("Composite IOD" = plot_palette$treatment, "Composite CV2" = plot_palette$control)) +
    labs(title = paste0(comparison_name, " | two-line regime crossing"), subtitle = compact_caption("Dashed = shared cutoff. Dotted = backup treatment and control crossings.", width = 90), x = "Percentile center", y = "Composite local amplitude", color = NULL) +
    manuscript_theme()
  if (is.finite(crossing_pct)) {
    p <- p + geom_vline(xintercept = crossing_pct, linetype = "dashed", linewidth = 0.95, colour = plot_palette$threshold) + annotate("label", x = crossing_pct, y = ymax, label = paste0("Shared cutoff\np = ", signif(crossing_pct, 4), "\nrank = ", crossing_rank), fill = "white", colour = plot_palette$threshold, size = 3.0, label.size = 0.15, vjust = -0.55)
  }
  if (is.finite(trt_pct)) {
    p <- p + geom_vline(xintercept = trt_pct, linetype = "dotted", linewidth = 0.85, colour = plot_palette$treatment) + annotate("label", x = trt_pct, y = ymax * 0.78, label = paste0("Treatment backup\np = ", signif(trt_pct, 4), "\nrank = ", trt_rank), fill = "white", colour = plot_palette$treatment, size = 2.7, label.size = 0.15)
  }
  if (is.finite(ctrl_pct)) {
    p <- p + geom_vline(xintercept = ctrl_pct, linetype = "dotted", linewidth = 0.85, colour = plot_palette$control) + annotate("label", x = ctrl_pct, y = ymax * 0.58, label = paste0("Control backup\np = ", signif(ctrl_pct, 4), "\nrank = ", ctrl_rank), fill = "white", colour = plot_palette$control, size = 2.7, label.size = 0.15)
  }
  p
}
...
plot_regime_difference_curve <- function(combined_cutoff_info, comparison_name) {
  ...
  ggplot(df, aes(percentile, regime_difference)) +
    ...
    labs(title = paste0(comparison_name, " | regime-difference curve"), subtitle = compact_caption("Positive = IOD dominant, negative = CV2 dominant. Last stable zero-crossing = EVS cutoff.", width = 90), x = "Percentile center", y = "IOD - CV2") +
    manuscript_theme()
}
...
build_crossing_summary_table <- function(combined_cutoff_info, comparison_name) {
  sc <- combined_cutoff_info$selected_crossing
  if (is.null(sc) || !nrow(sc)) return(data.frame(comparison_name = comparison_name, selected_reason = combined_cutoff_info$selected_reason, crossing_percentile = NA_real_, crossing_rank = NA_integer_, stringsAsFactors = FALSE))
  data.frame(comparison_name = comparison_name, selected_reason = combined_cutoff_info$selected_reason, crossing_id = sc$crossing_id[1], crossing_percentile = sc$crossing_percentile[1], crossing_rank = sc$crossing_rank[1], percentile_left = sc$percentile_left[1], percentile_right = sc$percentile_right[1], regime_difference_left = sc$regime_difference_left[1], regime_difference_right = sc$regime_difference_right[1], stringsAsFactors = FALSE)
}

build_backup_crossing_summary_table <- function(combined_cutoff_info, comparison_name) {
  add_row <- function(sc, label, reason) {
    if (is.null(sc) || !nrow(sc)) data.frame(comparison_name = comparison_name, cutoff_type = label, selected_reason = reason, crossing_percentile = NA_real_, crossing_rank = NA_integer_, stringsAsFactors = FALSE)
    else data.frame(comparison_name = comparison_name, cutoff_type = label, selected_reason = reason, crossing_percentile = sc$crossing_percentile[1], crossing_rank = sc$crossing_rank[1], stringsAsFactors = FALSE)
  }
  bind_rows(
...
run_core_analysis <- function(count_mat, coldata, dataset_name, annot_df) {
  ...
  res_df <- left_join(res_df, shr_df[, c("feature_id", "log2FoldChange", "lfcSE"), drop = FALSE], by = "feature_id", suffix = c("", "_shrunk"))
  colnames(res_df)[colnames(res_df) == "log2FoldChange_shrunk"] <- "lfc_shrunk"
  colnames(res_df)[colnames(res_df) == "lfcSE_shrunk"] <- "lfc_shrunk_SE"
  res_strong_df <- as.data.frame(res_strong, stringsAsFactors = FALSE); res_strong_df$feature_id <- as.character(rownames(res_strong_df))
  res_weak_df <- as.data.frame(res_weak, stringsAsFactors = FALSE); res_weak_df$feature_id <- as.character(rownames(res_weak_df))
  res_df <- left_join(res_df, res_strong_df[, c("feature_id", "padj"), drop = FALSE], by = "feature_id", suffix = c("", "_strong"))
  res_df <- left_join(res_df, res_weak_df[, c("feature_id", "padj"), drop = FALSE], by = "feature_id", suffix = c("", "_weak"))
  ...
  hc_empirical_info <- resolve_empirical_hc_threshold(res_df$empirical_p, dataset_name)
  ...
  res_df$effect_class <- case_when(res_df$deseq2_strong_call ~ "strong_effect", res_df$deseq2_weak_call ~ "weak_effect", TRUE ~ "intermediate")
  norm_counts <- as.data.frame(counts(dds, normalized = TRUE)); norm_counts$feature_id <- as.character(rownames(norm_counts))
  mm <- as.data.frame(mcols(dds), stringsAsFactors = FALSE); mm$feature_id <- as.character(rownames(mm))
  ...
  annot_df <- annot_df %>% mutate(gene_symbol = if_else(is.na(gene_symbol), "", trimws(gene_symbol))) %>% arrange(feature_id, desc(gene_symbol != ""), gene_symbol) %>% distinct(feature_id, .keep_all = TRUE) %>% mutate(gene_symbol = na_if(gene_symbol, ""))
  final_df <- res_df %>% left_join(annot_df, by = "feature_id") %>% left_join(norm_counts, by = "feature_id") %>% left_join(disp_df, by = "feature_id")
  ...
  preferred_cols <- c("dataset_name", "feature_id", "gene_symbol", "baseMean", "lfc_shrunk", "regulation_direction", "lfdr", "pvalue", "padj", "empirical_p", "empirical_q", "HBFSS", "hc_empirical_threshold_dataset", "hbfss_threshold_dataset", "resLA_padj", "resGA_padj", "hc_empirical_pass", "deseq2_strong_call", "deseq2_weak_call_raw", "deseq2_weak_call", "standard_significant", "HBFSS_significant", "effect_class")
  final_df <- final_df[, c(intersect(preferred_cols, names(final_df)), setdiff(names(final_df), preferred_cols)), drop = FALSE]
  list(dds = dds, results = final_df, base_mean_vec = final_df$baseMean[!is.na(final_df$baseMean)], hc_empirical_threshold = hc_empirical_threshold, hbfss_threshold = hbfss_threshold_dataset)
}

# -----------------------------------------------------------------------------
# VOLCANO HELPERS
# -----------------------------------------------------------------------------
volcano_class_colors <- c(background = plot_palette$background, weak = plot_palette$weak, strong = plot_palette$strong, hbfss = plot_palette$hbfss, overlap = plot_palette$overlap)
volcano_class_labels <- c(background = "Background", weak = "DC2 weak + HC", strong = "DC2 strong", hbfss = "HBFSS only", overlap = "DC2 strong + HBFSS")

build_plot_specific_volcano_classes <- function(df, plot_type = c("standard", "hbfss")) {
  plot_type <- match.arg(plot_type)
  df <- as.data.frame(df, stringsAsFactors = FALSE)
  required_cols <- c("gene_symbol", "lfc_shrunk", "neglog10_wald_pvalue", "neglog10_empirical_p", "deseq2_strong_call", "deseq2_weak_call", "HBFSS_significant", "HBFSS_only_call", "overlap_call")
  ...
  df$plot_y <- if (plot_type == "standard") df$neglog10_wald_pvalue else df$neglog10_empirical_p
  df$volcano_class <- case_when(df$overlap_call ~ "overlap", df$deseq2_strong_call ~ "strong", df$deseq2_weak_call ~ "weak", df$HBFSS_only_call ~ "hbfss", TRUE ~ "background")
...
select_volcano_labels <- function(df, y_col = "plot_y", n_labels = 20) {
  ...
  if (!nrow(df)) return(df[0, , drop = FALSE])
  pri <- case_when(df$volcano_class == "overlap" ~ 1L, df$volcano_class == "strong" ~ 2L, df$volcano_class == "weak" ~ 3L, df$volcano_class == "hbfss" ~ 4L, TRUE ~ 9L)
  yy <- suppressWarnings(as.numeric(df[[y_col]])); yy[!is.finite(yy)] <- -Inf
  hh <- suppressWarnings(as.numeric(df$HBFSS)); hh[!is.finite(hh)] <- -Inf
...
volcano_label_layer <- function(lab_df) {
  ...
  ggrepel::geom_text_repel(data = lab_df, aes(label = gene_symbol_plot), size = 1.95, seed = 1, max.overlaps = 35, force = 1.25, force_pull = 0.5, box.padding = 0.38, point.padding = 0.22, min.segment.length = 0, segment.alpha = 0.6, segment.size = 0.22)
}
...
add_hbfss_boundary_layer <- function(p, df) {
  ...
  boundary_df <- data.frame(lfc_shrunk = c(-rev(x_abs), x_abs), neglog10_empirical_p = c(rev(y_hyper), y_hyper), stringsAsFactors = FALSE)
  ...
  if (nrow(boundary_df)) {
    ...
    p <- p + geom_path(data = boundary_df, aes(x = lfc_shrunk, y = neglog10_empirical_p), inherit.aes = FALSE, linetype = "dashed", linewidth = LINE_WIDTH_BOUNDARY, colour = plot_palette$hbfss) + annotate("label", x = max_abs_lfc * 0.82, y = label_y, label = paste0("HBFSS = ", signif(threshold, 4), " / |LFC|"), fill = "white", colour = plot_palette$hbfss, size = 2.8, label.size = 0.15)
  }
  if (is.finite(hc_p_threshold) && !is.na(hc_p_threshold) && hc_p_threshold > 0 && hc_p_threshold < 1) {
    p <- p + geom_hline(yintercept = -log10(hc_p_threshold), linetype = "dotted", linewidth = LINE_WIDTH_BOUNDARY, colour = plot_palette$threshold) + annotate("label", x = 0, y = -log10(hc_p_threshold), label = paste0("HC empirical p = ", signif(hc_p_threshold, 4)), fill = "white", colour = plot_palette$threshold, size = 2.8, label.size = 0.15, vjust = -0.7)
  }
  p
}
...
plot_standard_volcano <- function(df, dataset_name) {
  ...
  hc_emp <- suppressWarnings(as.numeric(df$hc_empirical_threshold_dataset[1]))
  p <- ggplot(df, aes(lfc_shrunk, plot_y)) +
    ...
    labs(title = pretty_dataset_label(dataset_name), x = "Shrunken log2 fold change", y = expression(-log[10](Wald~p)), caption = compact_caption("Colored markers match the final call classes. Weak DC2 calls are additionally gated by the empirical HC threshold.")) +
    coord_cartesian(clip = "off") + plot_expand_xy() + manuscript_theme() + guides(color = guide_legend(override.aes = list(shape = 16, size = 3.8, alpha = 1)))
  if (is.finite(hc_emp) && !is.na(hc_emp) && hc_emp > 0 && hc_emp < 1) {
    p <- p + geom_hline(yintercept = -log10(hc_emp), linetype = "dotted", linewidth = LINE_WIDTH_BOUNDARY, colour = plot_palette$weak) + annotate("label", x = 0, y = -log10(hc_emp), label = paste0("HC empirical p = ", signif(hc_emp, 4)), fill = "white", colour = plot_palette$weak, size = 2.8, label.size = 0.15, vjust = -0.7)
  }
...
plot_publication_volcano_panel <- function(df, dataset_name) plot_hbfss_volcano(df, dataset_name)

plot_dispersion_panel_for_dataset <- function(df, dataset_name) {
  plot_df <- data.frame(baseMean = suppressWarnings(as.numeric(df$baseMean)), dispersion = suppressWarnings(as.numeric(df$dispersion)), class = build_plot_specific_volcano_classes(df, plot_type = "hbfss")$volcano_class, stringsAsFactors = FALSE)
  ...
  ggplot(plot_df, aes(baseMean, dispersion, color = class)) +
    ...
    manuscript_theme() + guides(color = guide_legend(override.aes = list(shape = 16, size = 3.8, alpha = 1)))
}
...
compute_dataset_pca_plot <- function(count_df, coldata, dataset_name, preprocessing = c("normalized", "raw_counts"), precomputed_matrix = NULL) {
  ...
  if (!is.null(precomputed_matrix)) x <- as.matrix(precomputed_matrix)
  ...
  ggplot(pca_df, aes(PC1, PC2, label = Sample, shape = Condition, fill = Condition)) +
    ...
    coord_cartesian(clip = "off") + manuscript_theme() + theme(legend.position = "none", plot.margin = margin(t = 12, r = 18, b = 16, l = 16))
}
...
build_pc1_feature_export <- function(analysis_results, annot_df, comparison_name, evs, tab_dir) {
  annot_df$feature_id <- as.character(annot_df$feature_id)
  annot_df$gene_symbol <- as.character(annot_df$gene_symbol)
  base_export <- annot_df %>% distinct(feature_id, .keep_all = TRUE)
  add_dataset_columns <- function(df, key, prefix) {
    ...
    left_join(df, src, by = "feature_id")
  }
  ...
  out <- left_join(out, load_tbl(evs$fit_trt$loading_table, "pc1_loading_trt_normalized"), by = "feature_id")
  out <- left_join(out, load_tbl(evs$fit_untrt$loading_table, "pc1_loading_ctrl_normalized"), by = "feature_id")
  out <- left_join(out, load_tbl(evs$fit_trt_raw$loading_table, "pc1_loading_trt_raw"), by = "feature_id")
  out <- left_join(out, load_tbl(evs$fit_untrt_raw$loading_table, "pc1_loading_ctrl_raw"), by = "feature_id")
...
save_cross_dataset_comparison_panels <- function(comparison_name, analysis_results, cmp_dir, dataset_list, coldata, normalized_dataset_list = NULL) {
  ...
  summary_table <- bind_rows(lapply(keys_present, function(k) {
    ...
    data.frame(comparison_name = comparison_name, dataset_key = k, dataset_label = unname(dataset_key_labels[k]), dataset_name = sm$dataset_name[1], n_features = sm$n_features[1], n_standard_significant = sm$n_standard_significant[1], n_HBFSS_significant = sm$n_HBFSS_significant[1], n_overlap = sm$n_overlap_significant[1], n_strong_effect = sm$n_strong_effect[1], n_weak_effect = sm$n_weak_effect[1], hc_empirical_threshold = sm$hc_empirical_threshold[1], hbfss_threshold = sm$hbfss_threshold[1], evs_fixed_top_n = evs_fixed_top_n, stringsAsFactors = FALSE)
  }))
...
run_full_comparison_pipeline <- function(comparison_name, count_matrix, coldata, annot_df) {
  ...
  for (nm in names(dataset_list)) {
    ...
    summary_row <- data.frame(comparison_name = comparison_name, dataset_name = full_dataset_name, n_features = nrow(df), hc_empirical_threshold = fit$hc_empirical_threshold, hbfss_threshold = fit$hbfss_threshold, n_standard_significant = sum(df$standard_significant, na.rm = TRUE), n_HBFSS_significant = sum(df$HBFSS_significant, na.rm = TRUE), n_overlap_significant = sum(df$standard_significant & df$HBFSS_significant, na.rm = TRUE), n_strong_effect = sum(df$effect_class == "strong_effect", na.rm = TRUE), n_weak_effect = sum(df$effect_class == "weak_effect", na.rm = TRUE), evs_fixed_top_n = evs_fixed_top_n, shared_cutoff_rank = evs$combined_cutoff_info$top_n_actual, shared_cutoff_method = evs$combined_cutoff_info$method, shared_cutoff_quantile = evs$fit_trt$cutoff_quantile, shared_selected_reason = evs$combined_cutoff_info$selected_reason, stringsAsFactors = FALSE)
...
comparison_inputs <- lapply(seq_len(nrow(comparison_table)), function(i) {
  prepare_comparison_data(comparison_name = comparison_table$comparison_name[i], group1_prefix = comparison_table$group1_prefix[i], group2_prefix = comparison_table$group2_prefix[i], WTTS_Seq = WTTS_Seq, meta_all = meta_all)
})
...
for (cmp in names(comparison_inputs)) {
  ...
  out <- tryCatch(
    run_full_comparison_pipeline(comparison_name = input_obj$comparison_name, count_matrix = input_obj$count_matrix, coldata = input_obj$coldata, annot_df = OrigID_Symbol),
    error = function(e) {
...
all_summaries <- if (length(all_summaries_list) > 0) bind_rows(all_summaries_list) else data.frame()
if (nrow(all_summaries) > 0) save_csv(all_summaries, file.path(output_dir, "all_comparisons_overall_summary.csv"))
if (length(failed_comparisons) > 0) save_csv(bind_rows(failed_comparisons), file.path(output_dir, "failed_comparisons.csv"))
runtime_seconds <- as.numeric(difftime(Sys.time(), run_start, units = "secs"))
run_manifest <- data.frame(analysis_name = analysis_name, runtime_readable = format_runtime_minutes(runtime_seconds), runtime_seconds = runtime_seconds, alpha_level = alpha_level, lfc_boundary = lfc_boundary, evs_fixed_top_n = evs_fixed_top_n, output_dir = output_dir, stringsAsFactors = FALSE)
...
cat("=====================================================\n\n")
if (nrow(all_summaries) > 0) print(all_summaries) else message("No comparison summaries were written. Check failed_comparisons.csv for the exact error message.")
