...
compute_pc1_loading_table <- function(value_df, sample_names, top_n = evs_fixed_top_n,
  ...
  x_log <- log2(x + 1)
  row_var <- apply(x_log, 1L, stats::var, na.rm = TRUE)
  keep_rows <- is.finite(row_var) & !is.na(row_var) & row_var > 0

  if (sum(keep_rows) < 5L) {
    stop("Fewer than 5 nonzero-variance features remain for PCA loading estimation.", call. = FALSE)
  }

  x_log <- x_log[keep_rows, , drop = FALSE]
  max_pcs <- min(5L, ncol(x_log))
  pca_fit <- prcomp(t(x_log), scale. = FALSE, rank. = max_pcs)
...
run_core_analysis <- function(count_mat, coldata, dataset_name, annot_df) {
  ...
  if (is.na(hc_p_threshold_dataset)) {
    ...
    res_df$HBFSS_significant <- ifelse(
      is.na(res_df$HBFSS),
      FALSE,
      (res_df$HBFSS >= hbfss_threshold_dataset) & res_df$hc_empirical_pass
    )
  }
...
build_reviewer_volcano_classes <- function(df) {
  ...
  required_cols <- c(
    ...
    "hc_empirical_pass", "wald_pvalue", "empirical_p", "HBFSS",
  ...
  if (nrow(boundary_df)) {
    p <- p +
      ...
      annotate(
        "label",
        x = max_abs_lfc * 0.82,
        y = min(max(y_hyper, na.rm = TRUE), max(df$neglog10_empirical_p, na.rm = TRUE) * 0.92),
...
plot_standard_volcano <- function(df, dataset_name) {
  ...
  p <- ggplot(df, aes(lfc_shrunk, neglog10_wald_pvalue)) +
    geom_point(
      aes(color = method_call_class),
      alpha  = POINT_ALPHA_PRIMARY,
      size   = POINT_SIZE_PRIMARY + 0.60,
      shape  = 16
    ) +
    geom_vline(
      xintercept = c(-lfc_boundary, lfc_boundary),
    ...
    scale_color_manual(values = method_call_colors, drop = FALSE, name = "Call class") +
    labs(
      ...
      caption = compact_caption(paste0(
        ...
        "Weak DC2 calls are colored only when the feature also passes the empirical HC gate used downstream. ",
  ...
  p <- add_hbfss_boundary_layer(p, df) +
    labs(
      ...
      caption = compact_caption(paste0(
        ...
        "Dotted horizontal line = HC empirical-p threshold. ",
        "Dashed hyperbola = HBFSS threshold. ",
        volcano_count_caption(df)
      ))
    ) +
    coord_cartesian(clip = "off") +
  ...
  p <- add_hbfss_boundary_layer(p, build) +
    labs(
      ...
      caption = compact_caption(paste0(
        ...
        "Both thresholds are shown explicitly: HC empirical-p floor and HBFSS hyperbolic boundary. ",
...
run_full_comparison_pipeline <- function(comparison_name, count_matrix, coldata, annot_df) {
  ...
  if (!is.null(evs$combined_cutoff_info$combined_wave_map) && nrow(evs$combined_cutoff_info$combined_wave_map)) {
    save_csv(evs$combined_cutoff_info$combined_wave_map, file.path(tab_dir, paste0(comparison_name, "_last_stable_crossing_map.csv")))
  }
