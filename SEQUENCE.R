...
#   3. Select one shared EVS cutoff per comparison from the last stable
#      positive-to-negative crossing of the combined treatment plus control
#      local IOD and local CV2 curves.
...
#   - The EVS shared cutoff is based on the last stable positive-to-negative IOD minus CV2 crossing.
...
label_stable_crossings <- function(diff_df, crossing_tbl, stability_window_n = crossing_stability_window_n) {
  ...
  out$crossing_direction <- NA_character_
  ...
  for (i in seq_len(nrow(out))) {
    ...
    left_neg_frac <- mean(left_vals < 0, na.rm = TRUE)
    right_pos_frac <- mean(right_vals > 0, na.rm = TRUE)

    out$left_window_positive_frac[i] <- left_pos_frac
    out$right_window_negative_frac[i] <- right_neg_frac

    if (is.finite(left_pos_frac) && is.finite(right_neg_frac) &&
        left_pos_frac >= 0.67 && right_neg_frac >= 0.67) {
      out$stable_crossing[i] <- TRUE
      out$crossing_direction[i] <- "into_leading_edge_boundary"
    } else if (is.finite(left_neg_frac) && is.finite(right_pos_frac) &&
               left_neg_frac >= 0.67 && right_pos_frac >= 0.67) {
      out$stable_crossing[i] <- TRUE
      out$crossing_direction[i] <- "out_of_leading_edge_boundary"
    } else {
      out$stable_crossing[i] <- FALSE
      out$crossing_direction[i] <- "unstable"
    }
  }
...
select_regime_shift_crossing <- function(combined_wave_df) {
  ...
  stable_into_tbl <- crossing_tbl[
    crossing_tbl$stable_crossing &
      crossing_tbl$crossing_direction == "into_leading_edge_boundary",
    ,
    drop = FALSE
  ]

  if (nrow(stable_into_tbl)) {
    selected <- stable_into_tbl[order(stable_into_tbl$crossing_percentile, decreasing = TRUE), , drop = FALSE][1, , drop = FALSE]
    reason <- "last_stable_crossing_into_leading_edge"
    return(list(
      diff_df = diff_df,
      crossing_table = crossing_tbl,
  ...
  any_into_tbl <- crossing_tbl[
    crossing_tbl$crossing_direction == "into_leading_edge_boundary",
    ,
    drop = FALSE
  ]

  if (nrow(any_into_tbl)) {
    selected <- any_into_tbl[order(any_into_tbl$crossing_percentile, decreasing = TRUE), , drop = FALSE][1, , drop = FALSE]
    reason <- "last_crossing_into_leading_edge_fallback"
    return(list(
      diff_df = diff_df,
      crossing_table = crossing_tbl,
      selected_crossing = selected,
...
resolve_combined_fourier_cutoff <- function(fit_trt_loading_tbl, fit_ctrl_loading_tbl, fixed_top_n = evs_fixed_top_n) {
  ...
  list(
    ...
    cutoff_quantile = cutoff_quantile,
    n_total = n_total,
    method = "last_stable_crossing_into_leading_edge",
...
plot_combined_fourier_wave_map <- function(combined_cutoff_info, comparison_name) {
  ...
  p <- ggplot(df, aes(percentile)) +
    ...
    labs(
      ...
      subtitle = compact_caption("Dashed = last stable positive-to-negative crossing, taken as the final entry into the leading-edge boundary. Dotted = backup treatment and control crossings.", width = 90),
      x = "Percentile center",
...
plot_regime_difference_curve <- function(combined_cutoff_info, comparison_name) {
  ...
  ggplot(df, aes(percentile, regime_difference)) +
    ...
    labs(title = paste0(comparison_name, " | regime-difference curve"), subtitle = compact_caption("Positive = IOD dominant, negative = CV2 dominant. The EVS cutoff is the last stable positive-to-negative zero-crossing, which marks the final boundary of the leading edge.", width = 90), x = "Percentile center", y = "IOD - CV2") +
    manuscript_theme()
}
...
run_full_comparison_pipeline <- function(comparison_name, count_matrix, coldata, annot_df) {
  ...
  if (!is.null(evs$combined_cutoff_info$combined_wave_map) && nrow(evs$combined_cutoff_info$combined_wave_map)) save_csv(evs$combined_cutoff_info$combined_wave_map, file.path(tab_dir, paste0(comparison_name, "_last_stable_crossing_map.csv")))
