...
#
# Purpose:
# This section creates a self-contained manuscript pipeline. It loads the
# required packages, imports the single WTTS count file, embeds only the
# essential metadata (id, condition), defines the four pairwise comparisons,
# and sets reusable helper functions.
#
# Why this matters:
# The user only needs to provide the master WTTS count matrix. Everything
# else needed for execution is embedded here.
#
# Design overview:
# The pipeline tests four circadian time-point pairs from WTTS-Seq
# (whole-transcriptome termination site sequencing) post-mortem rodent brain
# tissue. The unit of observation is a polyadenylation site (PAS), not a gene.
# OrigID and Symbol are therefore kept distinct throughout.
#
# Differential expression is modeled with design = ~ condition in every
# comparison. No donor blocking term or alternate design is used in this final
# manuscript pipeline.
#
...
# LFC boundary used for:
# (1) strong/weak effect hypothesis tests via lfcThreshold
# (2) HBFSS threshold anchor = abs(log10(hc_p_threshold_dataset)) * lfc_boundary
lfc_boundary <- 1.0

# Top-ranked PAS features retained per condition-specific eigenvector split
top_n_target <- 5000
...
# SHORT EXPORT NAMES
#
# All exports begin with Figure_ or Table_ so GitHub folders sort cleanly and
...
# hc_p_threshold_dataset : dataset-specific HC threshold from empirical p-values
# hbfss_threshold_dataset: abs(log10(hc_p_threshold_dataset)) * lfc_boundary
# HBFSS         : abs(apeglm-shrunken log2FC * log10(empirical_p))
# standard_significant : manuscript standard DESeq2-positive plotted call
# -----------------------------------------------------------------------------
...
plot_pc1_loading_rank <- function(loading_tbl, cutoff, dataset_label, group_label,
  ...
  ggplot(loading_tbl, aes(rank, pc1_loading_abs)) +
    ...
    annotate(
      ...
      y     = cutoff,
      label = paste0("Absolute PC1 loading cutoff for top ", top_n_used, " sites = ", signif(cutoff, 4), "\n", quantile_label),
...
plot_mean_histogram_panel <- function(df_means, base_mean_vec, dataset_name) {
  p1 <- ggplot(data.frame(x = safe_log10(df_means$mean_trt + 1)), aes(x)) +
    geom_histogram(bins = 50, fill = plot_palette$histogram, color = "white") +
    labs(title = paste(dataset_name, "Treatment mean"),
         x = "log10(mean + 1)", y = "Count") +
    manuscript_theme()

  p2 <- ggplot(data.frame(x = safe_log10(df_means$mean_untrt + 1)), aes(x)) +
    geom_histogram(bins = 50, fill = plot_palette$histogram, color = "white") +
    labs(title = paste(dataset_name, "Control mean"),
         x = "log10(mean + 1)", y = "Count") +
    manuscript_theme()

  p3 <- ggplot(data.frame(x = safe_log10(df_means$mean_all + 1)), aes(x)) +
    geom_histogram(bins = 50, fill = plot_palette$histogram, color = "white") +
    labs(title = paste(dataset_name, "Pooled mean"),
         x = "log10(mean + 1)", y = "Count") +
    manuscript_theme()

  p4 <- ggplot(data.frame(x = safe_log10(base_mean_vec + 1)), aes(x)) +
    geom_histogram(bins = 50, fill = plot_palette$histogram, color = "white") +
    labs(title = paste(dataset_name, "DESeq2 baseMean"),
         x = "log10(baseMean + 1)", y = "Count") +
    manuscript_theme()
...
run_core_analysis <- function(count_mat, coldata, dataset_name, annot_df) {
  ...
  disp_cols_available <- intersect(
    c("feature_id", "dispGeneEst", "dispFit", "dispersion",
      "dispIter", "baseMean", "dispOutlier"),
    colnames(mm)
  )
...
plot_standard_volcano <- function(df, dataset_name) {
  ...
  p <- ggplot(
    ...
    labs(
      ...
      caption = compact_caption(paste0(
        count_caption,
        "\nInterpretive tier encodes empirical-null vs intermediate HBFSS candidate vs classical strong effect  |  Dashed lines = ±", lfc_boundary,
        " and adjusted p-value ", alpha_level
      ))
    ) +
    coord_cartesian(clip = "off") +
    manuscript_theme() +
  ...
  p <- add_hbfss_inter_highlight(p, df, "lfc_shrunk", "neglog10_padj")
...
plot_hbfss_volcano <- function(df, dataset_name) {
  ...
  p <- add_hbfss_inter_highlight(p, df, "lfc_shrunk", "neglog10_empirical_p")
...
plot_composite_volcano <- function(df, dataset_name) {
  ...
  p <- add_hbfss_inter_highlight(p, df, "lfc_shrunk", "neglog10_padjc")
...
plot_publication_volcano_panel <- function(df, dataset_name) {
  ...
  p <- add_hbfss_inter_highlight(p, build, "lfc_shrunk", "neglog10_empirical_p")
  p <- add_overlap_outline_gg(p, build, "lfc_shrunk", "neglog10_empirical_p", size = 2.05, stroke = 0.65)
  if (nrow(top_df) > 0) {
    p <- p + volcano_label_layer(top_df)
  }
...
plot_dispersion_relationships <- function(df, dataset_name) {
  ...
  do.call(
    ...
    c(plots,
      ...
      top  = list(textGrob(paste(compact_title(pretty_dataset_label(dataset_name)), "Dispersion relationships"),
                           gp = gpar(fontface = "bold", cex = 1.2))))
  )
}
...
compute_dataset_pca_plot <- function(count_df, coldata, dataset_name,
  ...
  ggplot(pca_df, aes(PC1, PC2, label = Sample, shape = Condition, fill = Condition)) +
    ...
    coord_cartesian(clip = "off") +
    manuscript_theme() +
    guides(fill = guide_legend(override.aes = list(size = 3.0, shape = 21, colour = "white")),
           shape = guide_legend(override.aes = list(size = 3.0, fill = "grey70", colour = "white")))
}
...
run_full_comparison_pipeline <- function(comparison_name, count_matrix, coldata, annot_df) {
  ...
  for (nm in names(dataset_list)) {
    ...
    if (!is.null(p_hc)) {
      save_plot(p_hc, fig_file(fig_subdir, comparison_name, nm, "HCProfile"))
    }
    ...
    if (!is.null(disp_panel)) {
      save_grob(disp_panel, fig_file(fig_subdir, comparison_name, nm, "DispRel"),
                width = 14, height = 8)
    }
    ...
    save_grob(summary_panel, fig_file(fig_subdir, comparison_name, nm, "Summary"),
              width = 16, height = 18)
